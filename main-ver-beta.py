...
import io
import os
from deep_translator import DeeplTranslator  # Gunakan DeepL lewat deep-translator
...
def translate_in_chunks(text_list, max_words=MAX_WORDS):
    ...
    translator = DeeplTranslator(source='auto', target='id')  # Ganti dengan DeepL
    translated_list = []
    buffer = ""
    chunk_map = []
    word_count = 0
...
def safe_translate(translator, text, retries=3, delay=5):
    for attempt in range(retries):
        try:
            translated = translator.translate(text)
            return translated.split('\n')
        except Exception as e:
            print(f"❌ Error saat translate (percobaan {attempt+1}): {e}")
            time.sleep(delay)
    print("⚠️ Gagal translate setelah beberapa percobaan. Gunakan teks asli.")
    return text.split('\n')  # fallback
